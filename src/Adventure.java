...
public class Adventure {
    public static void start(Map map) {
        ...
        while (true) {
            ...
            if (map.getCurrentRoom().isDark()) {
                System.out.println("The room is dark you cant see anything");
                if (!map.getCurrentRoom().isVisited()){
                    map.getCurrentRoom().makeVisited();
                }
            } else {
                if (!map.getCurrentRoom().isVisited()) {
                    System.out.println("You are in " + map.getCurrentRoom().getName() + ", " + map.getCurrentRoom().getDescription());
                    map.getCurrentRoom().makeVisited();
                } else System.out.println("You are in " + map.getCurrentRoom().getName());
                map.getCurrentRoom().printConnectedRooms();
            }
